...
let isChatboxOpen = localStorage.getItem('isChatboxOpen') === 'true' || false; // Load from localStorage
let unreadMessageCount = parseInt(localStorage.getItem('unreadMessageCount')) || 0; // Load from localStorage
...
chatStyle.textContent = `
    ...
    #dialog-panel {
        position: fixed;
        bottom: 20px;
        right: 90px; /* Offset to the left of chatButton (60px width + 10px gap) */
        width: 250px;
        max-height: 100px;
        background: #fff;
        border-radius: 10px;
        ...
        align-items: center;
        padding: 10px;
        z-index: 999; /* Below chatButton and chatbox */
        opacity: 0;
...
function openChatbox() {
    ...
    if (chatbox && chatButton) {
        ...
        isChatboxOpen = true;
        localStorage.setItem('isChatboxOpen', 'true'); // Save state
        // Reset unread message count and notification
        unreadMessageCount = 0;
        localStorage.setItem('unreadMessageCount', unreadMessageCount.toString()); // Save to localStorage
        ...
        // Remove dialog panel if present
        const dialogPanel = document.getElementById('dialog-panel');
        if (dialogPanel) dialogPanel.remove();
        loadChatHistory();
...
function closeChatbox() {
    ...
    if (chatbox && chatButton) {
        ...
        isChatboxOpen = false;
        localStorage.setItem('isChatboxOpen', 'false'); // Save state
        // Update notification badge and title based on unread messages
        if (unreadMessageCount > 0) {
            chatButton.classList.add('has-unread');
            chatButton.setAttribute('data-unread', unreadMessageCount);
...
document.addEventListener('click', (event) => {
    ...
    const chatButton = document.getElementById('chatButton');
    const dialogPanel = document.getElementById('dialog-panel');
    if (chatbox && chatButton && chatbox.style.display !== 'none' && !chatbox.contains(event.target) && !chatButton.contains(event.target) && (!dialogPanel || !dialogPanel.contains(event.target))) {
        closeChatbox();
    }
});
...
function showNotification() {
    try {
        ...
        if (Notification.permission === "granted") {
            const notification = new Notification("New Message from AirWing", {
...
function requestNotificationPermission() {
    try {
        if ("Notification" in window && Notification.permission === "default") {
            Notification.requestPermission().then(permission => {
                console.log("Initial permission request result:", permission);
            }).catch(error => {
                console.error("Initial permission request error:", error);
            });
        }
    } catch (error) {
...
function sendMessage() {
    ...
    setTimeout(() => {
        ...
        // Play a sound when the AI message is added
        const audio = new Audio('sound.wav'); // Replace with your sound file URL
        audio.play().catch(function(error) {
            console.log("Audio play failed: ", error);
            if (error.name === 'NotAllowedError') {
                alert('Please interact with the page (e.g., click anywhere) to enable chat sounds.');
            }
        });

        // Show notification and dialog panel if chatbox is closed
        if (!isChatboxOpen) {
            unreadMessageCount++;
            localStorage.setItem('unreadMessageCount', unreadMessageCount.toString()); // Save to localStorage
            const chatButton = document.getElementById('chatButton');
            ...
            // Show dialog panel with AI avatar and message
            let dialogPanel = document.getElementById('dialog-panel');
            if (dialogPanel) dialogPanel.remove(); // Remove existing panel if any
            dialogPanel = document.createElement('div');
            dialogPanel.id = 'dialog-panel';
            dialogPanel.innerHTML = `
                <img src="inglogo.png" alt="AI Avatar" class="dialog-avatar" />
                <p class="dialog-message">${aiResponse}</p>
            `;
            document.body.appendChild(dialogPanel);
            // Trigger show animation
            setTimeout(() => dialogPanel.classList.add('show'), 5);
            // Remove dialog panel after 10 seconds
            setTimeout(() => {
                if (dialogPanel) {
                    dialogPanel.classList.remove('show');
                    setTimeout(() => dialogPanel.remove(), 300); // Wait for fade-out transition
                }
            }, 4000);

            // Schedule browser notification after 1 minute if chatbox remains closed
            ...
            window._notificationTimeout = notificationTimeout;

            // Blink page title softly for 5 seconds
            let blinkState = true;
            const blinkInterval = setInterval(() => {
                document.title = blinkState ? `${unreadMessageCount} New Message${unreadMessageCount > 1 ? 's' : ''}` : originalTitle;
                blinkState = !blinkState;
            }, 800); // Toggle every 800ms for a soft blink
            setTimeout(() => {
                clearInterval(blinkInterval);
                document.title = `(${unreadMessageCount}) ${originalTitle}`;
            }, 8000); // Stop after 5 seconds
        } else {
            document.title = originalTitle; // Ensure title is original when chatbox is open
        }
...
// Function to trigger audio and title blink for unread messages
function notifyUnreadMessages() {
    if (!isChatboxOpen && unreadMessageCount > 0) {
        // Play audio
        const audio = new Audio('sound.wav'); // Replace with your sound file URL
        audio.play().catch(function(error) {
            console.log("Audio play failed: ", error);
        ...
        // Blink page title softly for 8 seconds
        let blinkState = true;
        const blinkInterval = setInterval(() => {
            document.title = blinkState ? `${unreadMessageCount} New Message${unreadMessageCount > 1 ? 's' : ''}` : originalTitle;
            blinkState = !blinkState;
        ...
        setTimeout(() => {
            clearInterval(blinkInterval);
            document.title = `(${unreadMessageCount}) ${originalTitle}`;
        }, 8000); // Stop after 8 seconds and update title
    } else {
        document.title = originalTitle; // Reset to original title if chatbox is open or no unread messages
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (chatButton) {
        ...
        // Restore notification badge state and title on page load
        if (unreadMessageCount > 0) {
            chatButton.classList.add('has-unread');
            chatButton.setAttribute('data-unread', unreadMessageCount);
            document.title = `(${unreadMessageCount}) ${originalTitle}`;
    ...
    // Restore chatbox state on page load
    const chatbox = document.getElementById('chatbox');
    if (chatbox && chatButton) {
        if (isChatboxOpen) {
            chatbox.style.display = 'flex';
            chatButton.style.display = 'none';
            document.title = originalTitle; // Ensure original title when chatbox is open
            loadChatHistory();
        } else {
            chatbox.style.display = 'none';
            chatButton.style.display = 'flex';
            document.title = unreadMessageCount > 0 ? `(${unreadMessageCount}) ${originalTitle}` : originalTitle;
        }
    }
    ...
    setInterval(notifyUnreadMessages, 5 * 60 * 1000); // 5 minute interval
});
